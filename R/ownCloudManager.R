...
#'    be the default permissions set by the ownCloud server (by default 'all').
#'  }
...
#'   'all'. By default the permissions will be the default permissions set by the ownCloud server (by default 'all').
...
#'   Shares a resource (file or folder) as public link. The function returns the public link generated by ownCloud.
#'  }
#'}
#' 
  ...
  private = list(
    ...
    checkAPIAvailability = function(name, element){
      if(!private$capabilities[[element]]$api_enabled){
        errMsg <- sprintf("This method requires the 'ownCloud' %s to be enabled.", name)
